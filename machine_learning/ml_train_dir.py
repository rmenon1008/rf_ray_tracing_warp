...
def get_vec_from_map(local_map):
    # Find the index of the max value
    max_index = np.argmax(local_map.flatten())
...
for file in os.listdir(DATASET_PATH):
    ...
    outputs.append(get_vec_from_map(data["local_map"]))
...
with torch.no_grad():
    for i in range(5):
        ...
        # Convert to lists
        input = input.reshape((128)).tolist()
        output = output.reshape((9)).tolist()
        prediction = prediction.reshape((9)).tolist()
